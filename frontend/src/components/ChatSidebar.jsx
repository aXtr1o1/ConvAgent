    ...
    try {
      ...
      if (newId) {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('active-conversation-changed', { detail: { id: newId } }));
        }
      }
      const data = await listConversations(userId);
  ...
  const handleDeleteConversation = async (e, conversationId) => {
    ...
    try {
      ...
      if (conversationId === activeConversationId) {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('active-conversation-deleted', { detail: { id: conversationId } }));
        }
      }
      const data = await listConversations(userId);
      setConversations(Array.isArray(data.conversations) ? data.conversations : []);
      ...
      <div className="chat-sidebar-inner">
        <div className="chat-sidebar-new-chat-wrap">
          <button type="button" className="chat-sidebar-new-chat" onClick={handleNewChat}>
            ...
            <svg className="chat-sidebar-new-chat-bubble" width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="chat-sidebar-features">
          ...
          <nav className="chat-sidebar-nav">
            <NavLink to="/chat" className={({ isActive }) => `chat-sidebar-nav-item ${isActive ? 'active' : ''}`} end>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z" />
              </svg>
              <span>Chat</span>
            </NavLink>
            <NavLink to="/chat/archived" className={({ isActive }) => `chat-sidebar-nav-item ${isActive ? 'active' : ''}`}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.54 5.23l-1.39-1.68C18.88 3.21 18.47 3 18 3H6c-.47 0-.88.21-1.16.55L3.46 5.23C3.17 5.57 3 6.02 3 6.5V19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V6.5c0-.48-.17-.93-.46-1.27zM12 17.5L6.5 12H10v-2h4v2h3.5L12 17.5zM5.12 5l.81-1h12l.94 1H5.12z" />
              </svg>
              <span>Archived</span>
            </NavLink>
            <NavLink to="/chat/library" className={({ isActive }) => `chat-sidebar-nav-item ${isActive ? 'active' : ''}`}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  <div className="chat-sidebar-conv-list" role="list">
                    {conversations.map((c) => (
                      <div
                        ...
                        role="listitem"
                        className={`chat-sidebar-conv-item ${activeConversationId === c.conversation_id ? 'active' : ''}`}
                        onClick={() => handleSelectConversation(c.conversation_id)}
                        title={c.title}
                      >
                        <span className="chat-sidebar-conv-title">{c.title || 'New Conversation'}</span>
                        <button
                          type="button"
        ...
        <div className="chat-sidebar-beta">
          <svg className="chat-sidebar-beta-icon" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
            <path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z" />
          </svg>
          <div className="chat-sidebar-beta-text">
            <strong>BETA VERSION</strong>
            <p>CortexForge Diagnostics is in beta - AI-generated guidance should be verified with official service procedures.</p>
          </div>
        </div>
      </div>
    </aside>
